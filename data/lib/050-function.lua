...
if (isPlayer(cid)) then
   	if (amount == nil) then return false end
   		doPlayerAddExp(cid,amount*getExperienceStage(getPlayerLevel(cid), getVocationInfo(getPlayerVocation(cid)).experienceMultiplier))
   	end
   	return true
end

function checkAreaM(cid, from, to)
    local monsters = {}
...
function checkAreaP(cid, from, to)
    local monsters = {}
    for z = from.z, to.z do
        for y = from.y, to.y do
            for x = from.x, to.x do
