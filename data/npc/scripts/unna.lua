...
function creatureSayCallback(cid, type, msg)
	...
	elseif msgcontains(msg, 'mission') then
		...
		elseif (get(cid) == 17) then
			local function talk_and_set(cid, pid)
				...
				setPlayerStorageValue(cid, 100105,24)
			end
	...
	elseif msgcontains(msg, 'yes') then
		if (turn[pid] == 1) then
			...
			set(cid, 1)	-- joined
			doPlayerSetStorageValue(cid, 100164, 1)
		elseif (turn[pid] == 2) then
			selfSay("You hero of our tribe if bring back holy skull!", cid)
			set(cid, 2)	-- skull mission
			setPlayerStorageValue(cid, 100105,1)
		elseif (turn[pid] == 3) then 
		if (doPlayerTakeItem(cid, 11073, 1)) then
			selfSay("Me thank you much! All wisdom safe again now.", cid)
			set(cid, 4)	-- new mission
			setPlayerStorageValue(cid, 100105,3)
			doPlayerSetStorageValue(cid, 100164, 2)
		else
			npcHandler:say("There is no skull with you.",cid)
		end
		elseif (turn[pid] == 4) then
			...
			setPlayerStorageValue(cid, 100105,4)
		elseif (turn[pid] == 5) then
			if doPlayerRemoveItem(cid, skull_id, 1) then
			if getPlayerStorageValue(cid, 100105) == 14 then
				...
				doPlayerSetStorageValue(cid, 100164, 3)
			else
				selfSay("What?? where is the skull?", cid)
			end
			else
				selfSay("Come back when you show the skull around!", cid)	
			end
		elseif (turn[pid] == 6) then
			...
			setPlayerStorageValue(cid, 100105,15)
		elseif (turn[pid] == 7) then
			...
			if (beer > 0) then
				...
				set(cid, 19)	-- DONE!!!
				setPlayerStorageValue(cid, 100105,25)
				doPlayerSetStorageValue(cid, 100164, 4)
				AddStageExp(cid,75000)
			else
