...
function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, 'join') and getPlayerStorageValue(cid, config.storage_join) < 1 then
		if(getPlayerStorageValue(cid,config.storage_bb) < 1) then
			...
			npcHandler:say("After all you have passed the Dream Challenge to reach this place, which used to be the process of initiation in the past...",cid, 3500)
			npcHandler:say("So I ask you: do you wish to become a member of the ancient order of the Nightmare Knights, ".. getCreatureName(cid) .."?",cid, 6500)
	...
	elseif msgcontains(msg, 'yes') then
		if(talkState[talkUser] == 1) then
			...
			npcHandler:say("Do you still want to join our order?",cid, 2500)
		...
		elseif(talkState[talkUser] == 2) then
			...
			npcHandler:say('You can always ask me about your current rank and about the privileges the ranks grant to those who hold them.',cid,3000)
