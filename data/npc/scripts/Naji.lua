...
local function isValidMoney(money)
    if isNumber(money) == TRUE and money > 0 and money < 99999999999 then
        return TRUE
    end
    return FALSE
end
    ...
    elseif talkState[cid] == 2 then
        if msgcontains(msg, 'yes') then
            if doPlayerDepositMoney(cid, moneyTo[cid], 1) ~= TRUE then
                ...
                selfSay('Alright, we have added the amount of '..moneyTo[cid]..' gold to your balance. You can withdraw your money anytime you want to. Your balance is ' .. getPlayerBalance(cid) .. '.', cid)
            end
