...
void Player::updateInventoryWeight()
{
	...
	for(int32_t i = SLOT_FIRST; i < SLOT_LAST; ++i)
	{
		if(Item* item = getInventoryItem((slots_t)i)) {
			...
			if (i == SLOT_BACKPACK && it.capDecreasePercent) {
				double itemWeight = item->getWeight();
				inventoryWeight += ((itemWeight / 100) * (100 - it.capDecreasePercent));
			} else {
...
void Player::sendCancelMessage(ReturnValue message) const
{
	switch(message)
	{
		...
		case RET_HOUSEPROTECTED:
			sendCancel("Essa casa est� protegida, voc� n�o pode mover ou jogar items!");
			break;
...
{
    return hasCapacity(item, count, 0);
}

bool Player::hasCapacity(const Item* item, uint32_t count, int32_t slot) const
{
	...
	if (slot == SLOTP_BACKPACK) {
		const Item* backpack = getInventoryItem(static_cast<slots_t>(SLOT_BACKPACK));
		if (backpack != nullptr) {
			const ItemType& it = Item::items[backpack->getID()];
			if (it.capDecreasePercent > 0) {
				return ((itemWeight / 100) * (100 - it.capDecreasePercent)) <= getFreeCapacity();
			}
		}
	}
...
ReturnValue Player::__queryAdd(int32_t index, const Thing* thing, uint32_t count, uint32_t flags, Creature*) const
{
	...
	if(childOwner)
	{
		...
		if(skipLimit || hasCapacity(item, count, SLOTP_BACKPACK))
			return RET_NOERROR;
