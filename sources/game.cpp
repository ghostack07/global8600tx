	...
	uint16_t spriteId, int16_t fromStackpos, const Position& toPos, uint8_t count)
{
	...
	//std::cout << "[DEBUG] - ID:" << item->getID() << "hasExhaust: " << player->hasCondition(CONDITION_EXHAUST, EXHAUST_MOVE_POT) << std::endl;
	if (item && item->getID() == 2562 && fromPos.x == 0xFFFF && toPos.x != 0xFFFF && player->hasCondition(CONDITION_EXHAUST, EXHAUST_MOVE_POT)) {
		return false;
	}
	...
	// std::cout << "[DEBUG] - fromPlayer:" << (fromCylinder == player || fromPos.y & 0x40) << "toGround: " << (toPos.x == 0xFFFF) << "toPlayer: " << toPos.x != 0xFFFF << std::endl;
	
	if (item->getID() == 2562 && (fromPos.x == 0xFFFF) && toPos.x != 0xFFFF) {
		if (Condition * privCondition = Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_EXHAUST,  g_config.getNumber(ConfigManager::MOVE_POT_EXHAUST_CONFIG), 0, false, EXHAUST_MOVE_POT)) {
			player->addCondition(privCondition);
		}
	}
	...
	const std::string& text, uint32_t statementId)
{
	...
	if(player->getLevel() < 10) { // 200 level min to send pm.
	player->sendTextMessage(MSG_STATUS_SMALL, "You need level 10+ for send Private message.");
	return false;
	}
