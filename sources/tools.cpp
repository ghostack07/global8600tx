...
std::string formatDateEx(time_t _time/* = 0*/, std::string format/* = "%d %b %Y, %H:%M:%S"*/) {
    ...
    auto t = std::chrono::system_clock::to_time_t(std::chrono::system_clock::from_time_t(_time));
    std::tm tms = *std::localtime(&t);
    char buffer[100];
    strftime(buffer, sizeof(buffer), format.c_str(), &tms);
    return buffer;
}
