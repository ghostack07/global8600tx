	...
	int32_t subType/* = -1*/, bool addArticle/* = true*/)
{
	...
	else if(it.weaponType != WEAPON_NONE)
	{
		...
		if(it.hasAbilities())
		{
			...
			if(it.abilities->regeneration)
			{
				...
				bool alreadyHaveRegeneration = false;

				if (it.abilities->healthGain && it.abilities->healthTicks)
				{
					s << it.abilities->healthGain << " hp/" << (it.abilities->healthTicks / 1000) << "s";
					alreadyHaveRegeneration = true;
				}
				
				if (it.abilities->manaGain && it.abilities->manaTicks)
				{
					if (alreadyHaveRegeneration) {
						s << ", ";
					}
					s << it.abilities->manaGain << " mp/" << (it.abilities->manaTicks / 1000) << "s";
				}
			}
	...
	else if(it.armor || (item && item->getArmor()) || it.showAttributes)
	{
		...
		if (it.capDecreasePercent > 0) {
			uint32_t capDecreasePercent = it.capDecreasePercent;
			s << ", Cap. Decrease: " << capDecreasePercent << "%";
		}
		s << ")";
	}
	else if(it.isKey())
