...
void Spectators::handle(ProtocolGame* client, const std::string& text, uint16_t channelId)
{
	...
	if(text[0] == '/')
	{
		...
		{
			int64_t lastShowCommandSeconds = OTSYS_TIME() - lastShowCommand;
			if (lastShowCommandSeconds < 10000) {
				int64_t timeLeft = 10000 - lastShowCommandSeconds;
				std::string str("You need to wait ");
				str.append(std::to_string((timeLeft / 1000)));
				str.append(" seconds.");
				client->sendCreatureSay(owner->getPlayer(), MSG_PRIVATE, str.c_str(), NULL, 0);
				return;
			}

			lastShowCommand = OTSYS_TIME();
			std::stringstream s;
		...
		{
			int64_t lastNameCommandSeconds = OTSYS_TIME() - lastNameCommand;
			if (lastNameCommandSeconds < 10000) {
				int64_t timeLeft = 10000 - lastNameCommandSeconds;
				std::string str("You need to wait ");
				str.append(std::to_string((timeLeft / 1000)));
				str.append(" seconds.");
				client->sendCreatureSay(owner->getPlayer(), MSG_PRIVATE, str.c_str(), NULL, 0);
		...
		{
			int64_t lastAuthCommandSeconds = OTSYS_TIME() - lastAuthCommand;
			if (lastAuthCommandSeconds < 10000) {
				int64_t timeLeft = 10000 - lastAuthCommandSeconds;
				std::string str("You need to wait ");
				str.append(std::to_string((timeLeft / 1000)));
				str.append(" seconds.");
				client->sendCreatureSay(owner->getPlayer(), MSG_PRIVATE, str.c_str(), NULL, 0);
